...
*                                             输出在输入数字间的随机值；模式三：在点击开始后，可以按键抢答并显示按键序号。
*                                             音乐播放系统可以对音乐进行相关操作，烘托氛围。在各个模式开始时，音乐音量会降低至最低值，模式结束或其他情况下音乐回到设定值。
*                                             特殊的，三个模式中，当显示的最大值不超过值12（由硬件限制，该值可以更改）时，对应值的LED灯会点亮，模拟选手所在位置被高亮表示。
*
*                                             代码简介：
...
*                                             此外还有其他相关外设函数配置，包含LED、LED灯阵，蜂鸣器、矩阵按键、音乐播放系统。
*******************************************************************************/
...
void USART1_IRQHandler(void)//串口中断接收
{
    if(USART_GetITStatus(USART1,USART_IT_RXNE ) != RESET)
    {
        ...
        if (ucTemp[0]==0x0d)
        {
            MUSIC_ADD();
            music_lever++;
            if (music_lever>11) music_lever=11;
        ...
        if (ucTemp[0]==0x0e)
        {
            MUSIC_Decrease();
            music_lever--;
            if (music_lever<3) music_lever=3;
            count=0;
        }
...
int main(void)
{
    ...
    HMISendInit();
    MUSIC_INIT();
    Matrix_ssKey_Pin_Init();//各种初始化，串口，随机数发生器、串口屏、按键、音乐模块、延时。
    Delay_Us(500);
    while(1)
    {
        ...
        if (rev==1)                                                        //模式一准备
        {
            ...
            {
                MUSIC_SOUND_DOWN(music_lever,2);
                flag=1;                                                   //如果传进来的值符合情况不进入串口屏显示阶段，退出模式
            }
            Delay_Ms(50);                                                 //没有其他情况就进入串口屏显示阶段
        }
        else if (rev==2)                                                   //模式二准备
        {
        ...
        else if (rev==3)                                                //进入模式三准备
        {
            ...
            {
                flag=3;
                MUSIC_SOUND_DOWN(music_lever,2);//正常情况进入模式三的串口屏显示，符合退出情况就退出。
            }
        ...
        {                                                               //模式二处理数据方式多了一些步骤
            ...
            HMISends(tjcstr);
            HMISendb(0xff);
            if(max2<13&&max2>0)   LED_INIT(random);
    //            else LED_INIT(0);
        }
